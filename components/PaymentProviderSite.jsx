...
// ⚠️ WERSJA JAVASCRIPT (bez TypeScript) — zgodna z plikami .jsx
// Jeśli chcesz TypeScript, zmień rozszerzenie na .tsx i dodaj tsconfig.

// Dostępne motywy (klucze)
const THEME = {
  blue: {
...
export default function PaymentProviderSite() {
  ...
  const [theme, setTheme] = useState("blue"); // 'blue' | 'emerald' | 'violet'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-white to-slate-50 text-slate-900 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 dark:text-slate-100">
            ...
            {products.map((p) => (
              <div key={p.id} className="group rounded-2xl border bg-white dark:bg-slate-900 dark:border-slate-800 overflow-hidden hover:shadow-lg transition-all">
                <div className="relative aspect-[4/3] overflow-hidden">
                  <Image src={p.img} alt={p.name} fill sizes="(max-width: 1024px) 50vw, 25vw" className="object-cover group-hover:scale-105 transition-transform" unoptimized />
                  <div className="absolute left-3 top-3 text-[11px] bg-white/90 dark:bg-slate-800/80 border dark:border-slate-700 px-2 py-0.5 rounded-full">{p.badge}</div>
                </div>
                <div className="p-4">
                  <div className="font-medium">{p.name}</div>
                  <div className="text-slate-600 dark:text-slate-300 text-sm mt-0.5">Krótki opis produktu — zalety i zastosowanie.</div>
                  <div className="mt-3 flex items-center justify-between">
                    <div className="font-semibold">{p.price}</div>
                    <button onClick={addToCart} className="inline-flex items-center gap-2 rounded-xl border px-3 py-1.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-800 dark:border-slate-700">
      ...
      {/* Płatności */}
      <section className="py-16 bg-white dark:bg-slate-900" id="oferta">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h3 className="text-xl font-semibold">Obsługiwane metody płatności</h3>
          <p className="text-slate-600 dark:text-slate-300 mt-1">Oferuj popularne metody: karty, BLIK i portfele mobilne.</p>
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {payments.map((name) => (
              <div key={name} className="rounded-xl border bg-slate-50 dark:bg-slate-800 dark:border-slate-700 py-3 text-center text-sm">{name}</div>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* Kontakt */}
      <section className="py-16 bg-white dark:bg-slate-900" id="kontakt">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-8">
          <div>
            ...
            <form className="mt-6 grid grid-cols-1 gap-4" onSubmit={(e) => e.preventDefault()}>
              <input className="rounded-2xl border px-4 py-3 dark:bg-slate-900 dark:border-slate-700" placeholder="Imię i nazwisko" />
              <input className="rounded-2xl border px-4 py-3 dark:bg-slate-900 dark:border-slate-700" placeholder="Adres e-mail" type="email" />
              <textarea className="rounded-2xl border px-4 py-3 min-h-[120px] dark:bg-slate-900 dark:border-slate-700" placeholder="Napisz, czym się zajmujesz i czego potrzebujesz" />
              <button className={`rounded-2xl text-white px-5 py-3 text-sm hover:opacity-90 ${palette.accent}`}>Wyślij</button>
            </form>
          </div>
          <div className="rounded-2xl border p-6 bg-slate-50 dark:bg-slate-800 dark:border-slate-700">
            <div className="font-medium">Dane firmy</div>
            <div className="mt-2 text-sm text-slate-700 dark:text-slate-300">Payment Provider Sp. z o.o.</div>
            <div className="text-sm text-slate-700 dark:text-slate-300">ul. Opolska 15, 40-084 Katowice</div>
            <div className="text-sm text-slate-700 dark:text-slate-300">E-mail: <a href="mailto:office@paymentproviderglobal.com" className={`underline ${palette.hoverAccent}`}>office@paymentproviderglobal.com</a></div>
            <div className="text-sm text-slate-700 dark:text-slate-300">Pomoc: <a href="mailto:help@paymentproviderglobal.com" className={`underline ${palette.hoverAccent}`}>help@paymentproviderglobal.com</a></div>
          </div>
        </div>
      </section>

      {/* FOOTER */}
      <footer className="bg-slate-50 dark:bg-slate-900 border-t dark:border-slate-800 mt-16">
        <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-4 gap-8 text-slate-700 dark:text-slate-300">
          {/* Logo i opis */}
          <div>
            <div className="flex items-center space-x-2 mb-4">
              <Image src={`${BASE}/logo-pp.svg`} alt="PaymentProvider logo" width={32} height={32} />
              <span className="font-semibold text-lg text-slate-900 dark:text-white">PaymentProvider</span>
            </div>
          ...
          {/* O nas */}
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-white mb-3">Firma</h3>
            <ul className="space-y-2 text-sm">
              <li><a href="#o-nas" className={`${palette.hoverAccent}`}>O nas</a></li>
              <li><a href="#" className={`${palette.hoverAccent}`}>Kariera</a></li>
              <li><a href="#kontakt" className={`${palette.hoverAccent}`}>Kontakt</a></li>
            </ul>
          </div>

          {/* Oferta */}
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-white mb-3">Oferta</h3>
            <ul className="space-y-2 text-sm">
              <li><a href="#oferta" className={`${palette.hoverAccent}`}>Płatności online</a></li>
              <li><a href="#oferta" className={`${palette.hoverAccent}`}>SoftPOS</a></li>
              <li><a href="#oferta" className={`${palette.hoverAccent}`}>Integracje API</a></li>
            </ul>
          </div>

          {/* Polityka */}
          <div>
            <h3 className="font-semibold text-slate-900 dark:text-white mb-3">Informacje</h3>
            <ul className="space-y-2 text-sm">
              <li><a href="#" className={`${palette.hoverAccent}`}>Polityka prywatności</a></li>
              <li><a href="#" className={`${palette.hoverAccent}`}>Regulamin</a></li>
              <li><a href="#" className={`${palette.hoverAccent}`}>RODO</a></li>
            </ul>
          </div>
        </div>

        {/* Dolna linia */}
        <div className="border-t dark:border-slate-800 py-4 text-center text-sm text-slate-500 dark:text-slate-400">
